"""
T12b Screening Runner (Third Route) - reruns all 9 screening prompts with glm-4.7-flash via ZhipuAI.
Uses the zai-sdk with thinking disabled since glm-4.7-flash defaults to reasoning mode
...
SCREENING_DIR = PROJECT_ROOT / "artifacts" / "research_runs" / "screening_third_route"
...
# frozen from T10 matrix, except provider/model
PROVIDER = "zhipuai"
MODEL = "glm-4.7-flash"
TEMPERATURE = 0.0
MAX_TOKENS = 256
...
def call_zhipu(client, prompt, max_retries=3):
    last_error = None
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if attempt >= max_retries:
                raise RuntimeError(f"ZhipuAI call failed after {max_retries} retries: {exc}") from exc
            last_error = exc
...
def write_run_config(run_dir, prompt_id, prompt_path):
    config = {
        "screening_phase": "Stage_A_screening_third_route",
        "prompt_id": prompt_id,
...
def main():
    ...
    for prompt_id, rel_prompt_path in PROMPTS:
        ...
        try:
            for row in rows:
                ...
                raw_output, latency = call_zhipu(client, prompt)
    ...
    with open(SCREENING_DIR / "screening_third_route_results.json", "w", encoding="utf-8") as f:
